  ...
  return (
    <div className='grid md:grid-cols-2 gap-10 grid-cols-1'>
      <div className='flex flex-col'>
        ...
        {...register("accountType", { required: "لطفاً نوع حساب را انتخاب کنید" })}
      />
        {errors.accountType && (
          <p className='mt-1 text-sm tex-red-700'>{errors.accountType.message as string}</p>
        )}
      </div>
      <div className='flex flex-col'>
        ...
        {errors.initialDeposit && (
          <p className='mt-1 text-sm text-red-700'>{errors.initialDeposit.message as string}</p>
        )}
      </div>
      <div className='md:col-span-2 mt-4 bg-gray-50 border border-gray-200 rounded p-4 text-gray-700'>
       ...
       <p>
        ...
        <span className='text-green-600'>{initialDeposit ? initialDeposit.toLocaleString("fa-IR") : "0"}{" "} تومان</span>
       </p>
      </div>
    </div>
  )
