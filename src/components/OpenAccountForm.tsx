...
import { OpenAccountSchema, OpenAccountSchemaType } from "../schemas/openAccountSchema";
...
>
  <h2 className="text-2xl font-bold text-center text-gray-800 mb-8">📄 افتتاح حساب بانکی</h2>
  ...
  <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
    ...
    <div>
      ...
      <input
        ...
        className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
      />
      {errors.nationalCode && <p className="text-sm text-red-600 mt-1">{errors.nationalCode.message}</p>}
    </div>
    ...
    {/* Account Type */}
    <div>
      <label className="block mb-1 font-medium text-gray-700">نوع حساب</label>
      <select
    ...
    <div className="md:col-start-2">
      <label className="block mb-1 font-medium text-gray-700">مبلغ واریزی اولیه</label>
      <input
        type="number"
      ...
      />
      {errors.initialDeposit && <p className="text-sm text-red-600 mt-1">{errors.initialDeposit.message}</p>}
    </div>
  ...
  />
  <label className="ml-2 text-sm text-gray-700">شرایط و قوانین را می‌پذیرم</label>
</div>
  ...
  <div className="mt-6 bg-gray-50 p-4 rounded-lg text-sm text-gray-700 border">
    <p><strong>نوع حساب انتخابی:</strong> {selectedAccountType}</p>
    <p><strong>مبلغ وارد شده:</strong> {deposit?.toLocaleString("fa-IR")} تومان</p>
  </div>
</form>
